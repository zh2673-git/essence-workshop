---
name: 连载小说
description: 多章节连载小说创作与管理系统，支持世界观构建、人物设定、分章大纲、单章写作与连载状态跟踪。基于 v2 方法论：三阶合一（阶段1是什么·点出本质·本质定义先行 → 阶段2为什么·建立因果·含反常识根因 → 阶段3怎么做·展开方法·视角升级）+ 每章一个本质（核心概念内核）+ 顿悟触发机制（一句话点破+反常识根因+视角升级）+ 类-属性-方法-路由（由本质定义锚定）。
version: 2.0
scene: E2
---
...
**每章本质定义先行** → 世界观构建（按三阶合一展开）→ 人物设定 → 故事主线 → 分季/分章大纲 → 单章写作（植入顿悟触发器）→ 配图生成 → 连载推送 → 状态更新。
...
- **类-属性-方法-路由由本质定义锚定**：本质定义 → 故事类归属 → 属性展开（世界观/人物/冲突）→ 方法体现（叙事策略）→ 路由适配（章节载体），每一层必须能回溯到本质定义
