...
| 4 | 将 `scripts/pipelines/` 下的其他脚本（html/slides/pptx/notebook/markdown）按形式重组 | ✅ | 中 |
| 5 | 合并视频平台适配器与通用平台适配器到统一 `platforms/` 目录 | ✅ | 中 |
| 6 | 实现统一调度器 `dispatcher.py`，按 form + platform 路由并应用约束 | ✅ | 中 |
| 7 | 新增 markdown 形式与 obsidian 平台适配器 | ✅ | 中 |
| 8 | 逐步废弃旧的平台名入口，保留 alias | ⏳ | 低 |
| 9 | 更新 CLI 帮助与路由表 | ✅（视频部分） | 低 |
...
- 通用平台：`browser` / `wechat` / `office` / `jupyter` / `reveal` / `obsidian`
