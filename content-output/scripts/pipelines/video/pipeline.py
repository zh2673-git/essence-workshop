...
- --platform 指定目标平台（video-channel / bilibili / douyin）
- --pipeline 保留为方法别名（dsl / html_record / article_to_video）
"""
import argparse
import os
import sys
...
def main(argv=None):
    ...
    parser.add_argument('--platform', default='video-channel',
                        choices=['video-channel', 'bilibili', 'douyin'],
                        help='目标平台，决定默认分辨率')

    # 向后兼容：旧 --pipeline 作为方法选择器
    parser.add_argument('--pipeline', choices=['dsl', 'html_record', 'article_to_video'],
    ...
    # 统一走 dsl_video_pipeline.py，它负责根据 content_spec 路由 method 并应用 platform 约束
    ...
    elif args.pipeline == 'article_to_video' and args.url:
    ...
    if args.url and args.pipeline != 'article_to_video':
        dsl_argv.extend(['--url', args.url])
