...
> **教学notebook**：开发完成后，可通过 [../../format/notebook/](../../format/notebook/) 将项目代码转化为教学notebook。
...
> **方案文档模板**（首轮方案 v1 / 后续方案 vN 的格式、验证契约表格等）走 [../../format/markdown](../../format/markdown) 与 [templates/program.md](templates/program.md)。本步骤只规定编排调用与方案原则，具体文档模板由 format 与 templates 提供。

### 4.1 首轮方案（v1）

调用 format/markdown 按 [templates/program.md](templates/program.md) 模板生成方案 v1，包含：目标、核心架构、渐进式构建步骤、正交增量原则、验证契约（前置条件 P + 后置条件 Q + 不变量 I + 不可验证项）。

### 4.2 后续方案（vN, N≥2）

调用 format/markdown 生成方案 vN，包含：上轮问题、本轮改进、详细设计、验证契约变更、评估预期。v2+ 的方案必须基于上轮测试报告的问题。
...
> 核心代码实现（Ollama原生API调用、总结轮兜底等）详见历史实战案例（已归档）。本节只保留经验要点，不嵌入代码实现。
...
PI-Agent 6轮Ratchet Loop迭代实录（本地小模型编码Agent，6轮0回退，3.5x性能提升）— 历史案例已归档
