...
description: |
  ...
  白板JSON内容由format/whiteboard生成，口播稿由content-framework/essay生成。
...
> 将白板JSON+口播稿加载到WhiteboardCaster，完成录制与MP4导出。内容生成由format处理，本适配器只管录制平台。

## 输入与输出

| 项 | 说明 |
|----|------|
| 输入 | .whiteboard.json（来自format/whiteboard）+ broadcast-script.md（来自content-framework/essay） |
| 输出 | MP4视频（含镜头跟随+人像叠加） |
...
| 职责 | 归属 |
|------|------|
| 生成白板元素JSON（场景结构、文字、图形、箭头） | format/whiteboard |
| 生成口播稿（5镜头结构、口语化） | content-framework/essay |
...
```
1. 读取 .whiteboard.json（来自format/whiteboard）
2. 读取 broadcast-script.md（来自content-framework/essay，用作提词器）
...
- `scripts/whiteboard_generator.py` — 白板JSON生成器（**内容生成逻辑归属format/whiteboard**，本适配器仅复用其元素处理逻辑）
- `scripts/svg_to_png.py` — SVG→PNG渲染器（Playwright，中文字体正确），供 `svg_to_whiteboard.py` 调用
