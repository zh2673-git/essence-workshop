...
description: |
  ...
  输入：标准Markdown（来自format/markdown）。输出：Obsidian兼容.md。
...
| 项目 | 说明 |
|------|------|
| 输入 | 标准 Markdown（来自 format/markdown） |
...
```
1. 读取标准 Markdown 文件（来自 format/markdown）
...
Obsidian 适配逻辑归属本适配器实现（标准 Markdown → Obsidian 兼容格式：wiki-links、双链、callout、标签）。上游输入由 `format/markdown` 提供标准 Markdown，本适配器执行平台专属转换。
