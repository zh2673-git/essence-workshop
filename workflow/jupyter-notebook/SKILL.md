...
Cell结构（内容编排）由 format/notebook 按上层指定的模板生成，本适配器不定义cell内容结构，只负责平台兼容性处理（kernel/依赖/输出清理）。
...
格式管线脚本：[../../format/notebook/scripts/generator.py](../../format/notebook/scripts/generator.py)
