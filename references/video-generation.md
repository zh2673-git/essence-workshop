...
**画面服务于内容，技术服务于效率。** 视频号的本质是「用画面+声音传递信息」，不是做精美动画。v2 升级：Stage+Sprite时间切片模型实现元素级动画控制，镜头间幻灯片硬切+镜头内元素级运动叙事，BGM+Ducking混音提升观感，多格式导出满足不同场景，品牌素材协议保证视觉一致性，反AI slop规则确保视觉品味。
...
1. **类型下限**：镜头类型不得少于5种（11种可用类型中至少选5种）
2. **bullet上限**：bullet类型镜头不得超过总镜头数的40%
3. **title上限**：title类型镜头不得超过总镜头数的20%
4. **强制类型**：
   - 每篇文章必须有至少1个 quote（金句卡）
   - 每篇文章必须有至少1个 summary（总结卡）
   - 有数据/数字的内容必须用 stat 或 chart，不得塞进 bullet
   - 有对比/对立的内容必须用 compare，不得拆成两个 bullet
   - 有步骤/流程/推导链的内容必须用 steps，不得用 bullet 平铺
   - 有核心争议/设问的内容必须用 qa 或 focus，不得用 bullet
5. **主动造数据**：从文章中提取关键数字（面积、百分比、年限、倍数等）做成 stat 卡，即使原文只是文字叙述。没有显式数字时，可从逻辑中提炼（如"4种学说"→ stat: `4` + "种学说之争"）

#### 内容-类型适配规则（必须遵守）

| 内容特征 | 必须使用的类型 | 禁止使用的类型 |
|---------|--------------|--------------|
| 含数字/百分比/倍数 | stat 或 chart | bullet |
| A vs B 对立/对比 | compare | bullet |
| 步骤/流程/推导链 | steps | bullet |
| 核心争议/设问+回答 | qa 或 focus | bullet |
| 一句话核心观点/引用 | quote | bullet |
| 关键词+解释 | focus | bullet |
| 时间序列/发展历程 | timeline | bullet |
| 章节标题引入 | title | — |
| 要点列举（无上述特征时） | bullet | — |
| 全文结尾 | summary | — |
...
**核心理念：镜头间幻灯片硬切，镜头内元素级运动叙事。**

镜头之间采用幻灯片式硬切（场景不重叠，干净利落），镜头之内每个元素有独立的入场/退场节奏，形成层次丰富的运动叙事。

#### 两层动画体系

| 层级 | 作用 | 实现位置 |
|------|------|---------|
| **场景级过渡** | 镜头入场/退场的整体视觉效果 | `tick()` 的 `applyTransition()` + `sceneOpacity()` |
| **元素级时序** | 每个元素有独立的enter/exit时间，不跟随卡片同步 | `autoTimeline()` 的逐元素 exit_at 梯度 |

#### 元素级退场梯度

`autoTimeline(slide, slideIndex, totalSlides)` 为每个元素计算 exit_at 时，不再统一停在 `dur-0.5`，而是按元素角色分配梯度退场时间：

```
旧方式：所有元素 exit_at = dur - 0.5（卡片消失前统一退场）
新方式：bg exit_at = dur - 0.3, heading exit_at = dur - 0.2, item_0 exit_at = dur - 0.1...
        → 背景最先退场，标题次之，内容元素最后消失
```

**退场梯度设计**：背景元素退场最早（dur-0.3），标题次之（dur-0.15~0.2），内容元素最晚（dur-0.05~0.1），形成「从外到内」的自然消散。

#### 场景级过渡类型

| 过渡类型 | 入场效果 | 退场效果 | 适用镜头 |
|---------|---------|---------|---------|
| `morph` | 画面从1.05x微缩到1x | 画面从1x微缩放大到1.05x | title, compare, stat |
| `slideAndFade` | 画面从下方80px滑入 | 画面向上滑出80px | bullet, steps, chart, timeline |
| `crossfade` | 纯透明度渐入 | 纯透明度渐出 | quote, summary, focus |
| `flip` | X轴翻转入场 | X轴翻转退场 | qa |

#### 渲染循环逻辑

```
tick() 主循环：
1. 筛选当前活跃场景（enter ≤ elapsed < exit）
2. 对每个活跃场景：
   a. 计算 sceneOpacity（fadeIn 0.25s + fadeOut 0.2s）
   b. 如果正在入场 → applyTransition(入场变换)
   c. 如果正在退场 → applyTransition(退场变换)
   d. applyCameraMotion(摄像机运动)
   e. buildSprites + update + 渲染器绘制
3. 场景间不重叠，硬切切换
```

#### 视觉效果对比

| 场景 | 旧效果（统一退场） | 新效果（元素级退场） |
|------|-----------------|------------------|
| 标题卡退场 | 所有元素同时消失 | 背景先淡出，标题次之，装饰线最后消失 |
| 要点卡退场 | 所有卡片同时消失 | 标题先退，装饰线次之，要点卡片逐个淡出 |
| 金句卡退场 | 引号+文字同时消失 | 引号先退，卡片次之，文字最后消失 |
| 统计卡退场 | 环+数字同时消失 | 环形先退，数字次之，标签最后消失 |

#### ⚠️ 元素级叙事硬性规则

1. **元素退场必须在场景时长之内**：exit_at < dur，不跨场景残留
2. **退场梯度必须有序**：背景 → 标题/装饰 → 内容元素，从外到内
3. **场景间硬切不重叠**：一个场景完全退出后，下一个场景才入场
4. **场景级过渡作为锦上添花**：morph/slideAndFade/crossfade 增加退场花样，但不是必须
