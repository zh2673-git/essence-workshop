...
description: |
  ...
  支持6大场景：知识探索、人物蒸馏、项目开发、项目解析、内容输出、Skill优化。
  三层交付架构：元素层（原子资产）→ 管线层（按平台约束组装）→ 平台层（交付成品）。
  5条管线：公众号管线、视频号管线、HTML交互管线、演示管线、PPT管线。
  输出风格支持：论文风格、蒸馏Skill风格、对话风格，以及预留扩展。
  触发词：「探索XX」「蒸馏XX」「开发XX」「分析XX」「写公众号」「发布文章」「做视频」「视频号」「做PPT」「HTML交互」「优化skill」「skill评分」「达尔文」「darwin」。
  模糊需求也触发：「我想提升决策质量」「帮我理解XX」「设计一个XX系统」「帮我改改skill」。
---
...
| **F: Skill优化** | 「优化skill」「skill评分」「达尔文」「darwin」「帮我改改skill」 | 评估并优化Skill质量 | 评分卡+优化报告 | [workflows/F-skill-optimization.md](workflows/F-skill-optimization.md) |
