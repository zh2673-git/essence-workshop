...
配图由 `scripts/elements/svg_themes.py` 生成，支持 7 套视觉主题，根据文章内容自动匹配：

| 主题 | 标签 | 情绪关键词 | 适用场景 |
|------|------|-----------|---------|
| **dark**（深空） | 默认 | 技术、编程、AI、工程、算法、数据 | 技术类文章 |
| **warm**（暖阳） | 暖色 | 温暖、生活、教育、成长、人文 | 生活/情感类 |
| **minimal**（极简） | 极简 | 极简、设计、美学、哲学、禅 | 设计/哲学类 |
| **nature**（自然） | 自然 | 自然、生态、中医、养生、本草 | 中医/自然类 |
| **ink**（水墨） | 国风 | 水墨、国风、传统、书法、诗词 | 传统文化类 |
| **cyber**（赛博） | 赛博 | 赛博、未来、黑客、元宇宙、Web3 | 科技前沿类 |
| **indigo**（靛蓝） | 认知 | 认知、思维、本质、深度、智慧 | 认知/思维类 |

**自动匹配逻辑**：`match_theme()` 函数扫描文章标题+副标题中的情绪关键词，命中数最多的主题胜出。无命中时默认 dark。

**语义色板系统**：每个主题定义 `semantic` 字段，包含 6 个语义化颜色，确保装饰元素跨主题适配：

| 语义色 | 用途 | 示例（dark 主题） |
|--------|------|-----------------|
| `item_accent_a` | 列表项奇数色/主强调 | `#EF6461` |
| `item_accent_b` | 列表项偶数色/副强调 | `#58C4DC` |
| `connector_color` | 连接线/箭头色 | `#E4B363` |
| `badge_bg` | 序号/标签背景 | `rgba(239,100,97,0.12)` |
| `divider_accent` | 分割线强调色 | `#EF6461` |
| `highlight_ring` | 高亮圆环色 | `#EF6461` |

> 渲染函数和装饰器通过 `_s(theme, key)` 统一取色，新增主题只需定义 `semantic` 字段即可自动适配所有渲染函数。

**装饰系统**：7 种装饰风格通过 `_DECOR_RENDERERS` 路由分发，装饰器内部使用语义色板取色：

| 装饰风格 | 主题 | 视觉特征 |
|---------|------|---------|
| circles | dark | 圆环+十字星+网格点阵 |
| dots | warm | 圆点+曲线+菱形 |
| lines | minimal | 线框矩形+十字线+菱形 |
| organic | nature | 椭圆+波浪曲线+叶片 |
| brush | ink | 墨晕+笔触+印章方块 |
| neon | cyber | 六边形+折线+三角 |
| constellation | indigo | 星座连线+星点+思维轨迹 |

**手动指定**：生成配图时可通过 `svg_theme` 参数指定主题，如 `render_svg_cover(..., theme_name="ink")`。

**排版主题与配图主题的关系**：排版主题（essence/claude-warm/claude-clean）控制文章样式，配图主题控制 SVG 图片风格，两者独立选择。默认根据文章内容自动匹配配图主题，不与排版主题绑定。

### 配图类型选择指引

每篇公众号文章配图 7 张（6 PNG + 1 GIF）。根据文章内容选择渲染函数，**不要生成封面图**（封面由微信后台单独上传）。

**基础渲染函数**：

| 内容类型 | 渲染函数 | 参数 | 说明 |
|---------|---------|------|------|
| 列举要点（3-7条） | `render_svg_card` | title, items[] | 每项最多30字 |
| 关键数据/数字 | `render_svg_stat` | value, label, sublabel | 大数字+标签 |
| 金句/引言 | `render_svg_quote` | text, source | 引言最多200字 |
| A vs B 对比 | `render_svg_compare` | title, leftTitle, rightTitle, left[], right[] | 左右两栏各5条 |
| 时间线/演进 | `render_svg_timeline` | title, events[{year,title,desc}] | 最多6个事件 |
| 步骤/流程 | `render_svg_steps` | title, steps[{title,desc}] | 最多6步 |
| 核心概念聚焦 | `render_svg_focus` | keyword, explanation | 关键词12字+解释60字 |
| 数据可视化 | `render_svg_chart` | title, data[{label,value}] | 横向柱状图，最多6条 |
| 总结/清单 | `render_svg_summary` | title, items[] | 编号列表，最多7条 |
| 问答 | `render_svg_qa` | question, answer | Q40字+A80字 |
| 核心概念详解 | `render_svg_feature` | title, features[{keyword,desc}] | 左关键词+右说明，4组 |
| 多维对比/多要点 | `render_svg_grid` | title, cards[{title,desc}] | 2×2或2×3卡片网格 |
| 趋势对比/折线图 | `render_svg_line_chart` | title, labels[], datasets[{name,values[]}] | 多数据线对比，最多3条线 |
...
| 循环/辩证过程 | `render_svg_cycle` | title, phases[{label,desc}] | 环形布局展示循环/辩证过程 |
| 列表详情 | `render_svg_list_detail` | title, items[{keyword,desc}] | 关键词+描述，比card信息密度更高 |
| 仪表盘 | `render_svg_dashboard` | title, metrics[], bar_data[], list_items[] | 多指标+柱状图+列表，数据密集 |
| 散点图 | `render_svg_scatter` | title, data[{label,x,y,group}] | 二维分布/象限分析 |
| 热力图 | `render_svg_heatmap` | title, data[{row,col,value}] | 矩阵关系/交叉分析 |
| 指标网格 | `render_svg_metric_grid` | title, metrics[], cols | 多指标卡片网格 |
| 柱状图 | `render_svg_bar_chart` | title, data[], labels[] | 竖向柱状图 |
| **自由绘制** | `render_svg_custom` | title, svg_body | LLM根据内容自由绘制SVG，模板仅提供框架 |

**配图规划原则**：
1. **以 `render_svg_custom` 自由绘制为主，预设模板为保底**——每张图优先考虑用custom根据内容定制绘制，只有当内容恰好是标准信息结构（纯数据图表、简单对比等）时才用预设模板
2. **每篇文章至少5张使用 `render_svg_custom`**，最多2张用预设模板
3. **禁止连续使用相同类型的渲染函数**——相邻两张图必须是不同类型
4. 引言类内容用 `quote`，数据类内容用 `stat`/`chart`/`dashboard`，逻辑推导用 `logic_flow`，辩证过程用 `cycle`
5. 文章开头用 `composite` 或 custom 概览（替代hero），结尾用 `quote` 或 custom 收束
6. **hero 仅用于封面，不在正文中使用**（hero卡片信息密度低，留白过多）
7. `grid` 类型文字容易溢出，优先用 `card` 或 `list_detail` 替代；如必须用 `grid`，每项文字不超过15字
8. **GIF动画**：使用HTML+CSS动画录制，分辨率800×500@1.5x（1200x750），帧数≥8，帧延迟200ms，最终GIF不超过2MB。**GIF防闪烁四原则**：
   - **禁用CSS transition/animation**：`stepFrame()` 中直接设置最终样式，不使用过渡动画（截取到中间帧会导致半透明混合色闪烁）
   - **禁用渐变和半透明色**：所有颜色必须用纯色（`#RRGGBB`），禁止 `rgba()`、`linear-gradient`、`radial-gradient`（GIF仅256色，渐变/半透明会产生严重色彩抖动）
   - **统一全局调色板**：录制脚本用所有帧采样生成统一调色板，`optimize=False`，`disposal=1`（避免帧间调色板突变闪烁）
   - **背景色用 `gif_bg`**：每个主题的 `bg` 字典中有 `gif_bg` 字段，HTML动画的 `body background` 必须用 `gif_bg`（当前 `gif_bg` = `bg1`，因为SVG背景色修复后PNG背景色即为 `bg1`）

**SVG渲染注意事项**：
   - **禁止使用 `<solidColor>`**：Chromium不支持SVG 1.2的 `<solidColor>` 元素，渲染为白色。必须用 `<linearGradient>` 两个相同stop-color模拟纯色
   - **grain滤镜opacity必须≤0.05**：`feBlend mode="overlay"` 在深色背景上会严重变暗，opacity=0.5时 `#2B2D42` → `#151620`，必须用0.05

**自由绘制策略（render_svg_custom）—— 主力绘图方式**：

`render_svg_custom(title, svg_body, theme_name, width, height)` 是解决模板单调性的核心方案。

**设计理念**：预设模板是"保底策略"，保证基本视觉质量；custom是"主力策略"，追求每张图的内容定制化表达。

**custom绘图类型参考**：
- 概念关系图：中心节点+分支连线+标签卡片（如哲学三大领域关系）
- 思维导图：树状层级结构，根节点→子节点→叶节点
- 流派演化图：时间轴+分支+交汇，展示思想流变
- 人物关系网络：节点+连线+标签，展示人物间影响关系
- 概念层级树：金字塔/嵌套结构，展示概念包含关系
- 对话/辩证图：对立面+中间地带，展示思想张力
- 过程/阶段图：自定义路径+节点，展示思维发展过程

**svg_body 规则**：
- 内容区可用范围：x=35~width-35, y=85~height-40
- 使用主题色板取色：`_s(get_theme(theme_name), "item_accent_a")` 等
- 文字字体：`font-family="PingFang SC, Microsoft YaHei, sans-serif"`
- 禁止 `<foreignObject>`、外部图片/字体
- 超出内容区的部分会被自动裁剪
- 充分利用SVG能力：`<path>` 绘制曲线/箭头，`<linearGradient>` 渐变填充，`<filter>` 阴影/模糊
```
