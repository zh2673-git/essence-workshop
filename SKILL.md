...
**配图规划原则**：
1. 每篇文章至少覆盖 3 种不同渲染函数，避免视觉单调
2. 优先使用 `feature` 和 `grid` 组合布局，信息密度更高
3. 引言类内容用 `quote`，数据类内容用 `stat` 或 `chart`，步骤类内容用 `steps` 或 `timeline`
4. 文章开头用 `card` 或 `grid` 概览要点，结尾用 `summary` 总结
```
