...
> **配图内容**：SVG/PNG/GIF 配图内容由 format-pipeline 生成，本适配器只做平台格式检查（尺寸/DPI/大小）与素材库上传。
...
配图内容（SVG/PNG/GIF）由 format-pipeline 生成，本适配器只做平台格式检查与上传。平台约束如下：
...
SVG 配图的画幅比例（配图内容由 format-pipeline 生成，本适配器检查画幅是否符合平台约束）：
