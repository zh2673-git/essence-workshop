...
# v3.0 职责分层：Prompt组件来自format-pipeline，方法论来自cognitive-engine，本文件只负责执行
PROMPT_GUIDE_PATH = PROJECT_ROOT / "format-pipeline" / "whiteboard" / "references" / "ai-prompt-guide.md"
METHODOLOGY_PATH = PROJECT_ROOT / "cognitive-engine" / "general-logic" / "references" / "methodology.md"
ELEMENT_SPEC_PATH = SKILL_DIR / "references" / "excalidraw-element-spec.md"

def load_system_prompt():
    """加载系统Prompt：拼接白板格式Prompt（format-pipeline）和认知方法论（cognitive-engine）。
    本函数不重定义任何方法论或Prompt设计，仅做读取和拼接。"""
    with open(PROMPT_GUIDE_PATH, "r", encoding="utf-8") as f:
        prompt_guide = f.read()
    with open(METHODOLOGY_PATH, "r", encoding="utf-8") as f:
        methodology = f.read()
    return f"{prompt_guide}\n\n---\n\n## 方法论详细内容（引用自cognitive-engine）\n\n{methodology}"


def fetch_content(source: str) -> dict:
    """获取内容：支持本地md文件路径、公众号URL、纯文本"""
    if source.startswith("http://") or source.startswith("https://"):
        sys.path.insert(0, str(PROJECT_ROOT / "platform-adapter" / "wechat" / "scripts"))
