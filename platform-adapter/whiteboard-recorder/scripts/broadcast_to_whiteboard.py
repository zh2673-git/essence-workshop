"""
本质工坊 · 白板录制项目生成器
输入：预生成的白板内容 JSON（由 format-pipeline 生成，含场景和元素）
...
注意：白板元素内容由 format-pipeline 生成，本脚本不做 LLM 内容生成。
"""
import argparse
import json
import re
...
def main():
    ...
    parser.add_argument("whiteboard_json", help="预生成的白板内容 JSON 文件（由 format-pipeline 生成，含场景和元素）")
