...
迭代开发统一使用**项目代码模式**（{project-root} 占位符）：
...
> **教学notebook**：开发完成后，可通过 [../../format-pipeline/notebook/](../../format-pipeline/notebook/) 将项目代码转化为教学notebook。
...
> **方案文档模板**（首轮方案 v1 / 后续方案 vN 的格式、验证契约表格等）走 [../../format-pipeline/markdown](../../format-pipeline/markdown) 与 [templates/program.md](templates/program.md)。本步骤只规定编排调用与方案原则，具体文档模板由 format-pipeline 与 templates 提供。

### 4.1 首轮方案（v1）

调用 format-pipeline/markdown 按 [templates/program.md](templates/program.md) 模板生成方案 v1，包含：目标、核心架构、渐进式构建步骤、正交增量原则、验证契约（前置条件 P + 后置条件 Q + 不变量 I + 不可验证项）。

### 4.2 后续方案（vN, N≥2）

调用 format-pipeline/markdown 生成方案 vN，包含：上轮问题、本轮改进、详细设计、验证契约变更、评估预期。v2+ 的方案必须基于上轮测试报告的问题。
...
### 6.1 三维验证（纯工具产品）/ 四维验证（协同产品）

验证不是单点测试，而是三个维度（纯工具产品）或四个维度（协同产品）的全链路验证：
...
1. **validate_pre()**：前置条件检查（进入执行前）
   ...
   - 基线是否锁定？
   - 测试脚本是否可运行？
   - 【协同产品】LLM API / 本地模型是否可访问？Prompt 模板是否就绪？
...
3. **validate_inv()**：不变量检查（每轮必须）
   ...
   - 对外接口是否兼容？
   - program.md约束是否被违反？
   - 【协同产品】Prompt 版本是否回退？符号规则是否被绕过？

4. **validate_llm()**（协同产品专属）
   - LLM 输出可解析率是否达标？
   - 符号校验通过率是否达标？
   - 幻觉检出率是否达标？
   - 失败降级成功率是否达标？
...
validate_post() ✅ AND validate_inv() ✅ AND validate_llm() ✅ → 保留（commit）
任一维度 ❌ → 回退（具体分析失败维度）
```
...
| ✅ | ✅ | ✅ | ❌ | **回退** | `git revert HEAD` + 记录 LLM 维度失败原因（如 Prompt 漂移、校验率下降） |
| ✅ | ❌ | ❌ | ❌ | **回退** | `git revert HEAD` + 记录多重失败原因 |

> **关键洞察**：协同产品中，L 维度失败是最容易被忽视的——LLM 输出看似正常，但可解析率、校验通过率或降级成功率下降，意味着系统在真实运行时可能失控。
...
```tsv
experiment_id	timestamp	change_description	pre_post	post_result	inv_result	llm_result	kept
001	2026-06-16T10:00	[改动描述]	✅	✅	✅	✅	true
002	2026-06-16T10:15	[改动描述]	✅	✅	❌	✅	false
003	2026-06-16T10:30	[改动描述]	✅	✅	✅	✅	true
004	2026-06-16T10:45	[改动描述]	✅	✅	✅	❌	false
```

> 纯工具产品可省略 `llm_result` 列，或始终填 ✅。
...
| P 前置 | 环境一致 | ✅/❌ | |
| P 前置 | 基线锁定 | ✅/❌ | |
| P 前置 | 【协同产品】LLM API 可访问 | ✅/❌ | |
| Q 后置 | 功能正确 | ✅/❌ | |
| Q 后置 | 指标改善 | ✅/❌ | |
| I 不变量 | 旧功能通过 | ✅/❌ | |
| I 不变量 | 接口兼容 | ✅/❌ | |
| I 不变量 | 【协同产品】Prompt 版本未回退 | ✅/❌ | |
| L LLM | 【协同产品】可解析率达标 | ✅/❌ | |
| L LLM | 【协同产品】校验通过率达标 | ✅/❌ | |
| L LLM | 【协同产品】幻觉检出率达标 | ✅/❌ | |
| L LLM | 【协同产品】降级成功率达标 | ✅/❌ | |
...
| 陷阱 | 症状 | 通用解决方案 |
|------|------|-------------|
| Prompt 漂移 | 更新 Prompt 后 LLM 行为不稳定 | Prompt 版本化；A/B 测试；小流量验证 |
| 校验规则过严 | LLM 输出频繁被拦截，迭代收敛慢 | 规则分级；先 soft 后 hard；人工审核 |
| 幻觉漏检 | 符号系统未拦截到事实错误 | 增加事实溯源规则；人工抽检 |
| 降级失效 | LLM 不可用时规则兜底也失败 | 设计多级降级；保留纯规则分支 |
| 模型成本失控 | 每次调用都用强模型 | 模型路由；轻量模型处理简单任务；缓存 |
...
> 核心代码实现（Ollama原生API调用、总结轮兜底等）详见 [../../cases/pi-agent-iteration.md](../../cases/pi-agent-iteration.md) 实战案例。本节只保留经验要点，不嵌入代码实现。

- **Ollama原生API调用**：通过 `/api/chat` 端点，`think` 参数控制是否返回 thinking 内容；当 content 为空但 thinking 有内容时，回退使用 thinking
- **总结轮兜底**：工具调用后若无 content，追加一轮总结调用（think=False, max_tokens=200）生成简洁回答
