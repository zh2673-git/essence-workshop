...
1. **自动检测 section**：扫描 HTML 中的 `.dimension-card`、`.group`、`.question-overview` 等语义元素，确定滚动停止点
2. **按 section 停留**：每个 section 停留时间由对应 TTS 旁白时长决定，无旁白则默认 5 秒
3. **平滑滚动**：使用 `scrollTo({behavior: 'smooth'})` 实现自然滚动
4. **首尾处理**：开头停留 1 秒，结尾回到顶部停留 2 秒
...
- **口播稿管线（broadcast）**：提供脚本来源（口播稿 → 视频旁白）
