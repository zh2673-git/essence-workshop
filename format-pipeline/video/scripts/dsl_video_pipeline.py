...
新模型：
- form = 'video'
- method = 'dsl' / 'html_record' / 'article_to_video'（由内容特征路由）
"""
import argparse
import json
import os
...
def run_article_to_video_pipeline(url: str, output_dir: str):
    wechat_scripts_dir = os.path.abspath(os.path.join(os.path.dirname(__file__), '..', '..', '..', 'platform-adapter', 'wechat', 'scripts'))
    sys.path.insert(0, wechat_scripts_dir)
...
def generate_video(content_spec: dict, output_dir: str, entry_file: str = '',
    ...
    if method == 'dsl':
        ...
        if not url:
            raise ValueError('网页文章转视频管线需要提供文章 URL')
        video_path = run_article_to_video_pipeline(url, output_dir)
    else:
        raise ValueError(f"Unknown method: {method}")

    return {'method': method, 'video': video_path, 'decision': decision}
...
def main(argv=None):
    ...
    parser.add_argument('--url', help='网页文章 URL（article_to_video 分支）')
    ...
    print(f"  [Done] {result['method']}: {result['video']}")
