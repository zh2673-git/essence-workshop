...
description: |
  ...
  负责生成Excalidraw兼容的白板元素结构，不含录制功能（录制由platform-adapter/whiteboard-recorder负责）。
...
> 生成白板场景JSON。本管线只负责"画什么"，"怎么录"由平台适配层处理。

## 与平台适配层的分工

| 职责 | 归属 |
|------|------|
| 生成白板元素JSON（场景结构、文字、图形、箭头） | **本管线**（format-pipeline/whiteboard） |
| 白板渲染、镜头跟随、提词器、录制MP4 | platform-adapter/whiteboard-recorder |
...
- [platform-adapter/whiteboard-recorder/SKILL.md](../../platform-adapter/whiteboard-recorder/SKILL.md) 的元素约束
...
白板JSON生成逻辑归属本管线。`platform-adapter/whiteboard-recorder/scripts/whiteboard_generator.py` 的内容生成部分归属本管线，平台适配器仅复用其元素处理逻辑做渲染验证。
