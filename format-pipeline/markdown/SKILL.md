...
description: |
  ...
  最通用的跨平台管线，是其他管线的文本来源。产出标准Markdown，平台特定适配（如Obsidian wiki-links）由平台适配层处理。
...
> 将认知产物的文本与图形元素合并为标准 Markdown 文档。作为最通用的跨平台管线，是其他管线的文本来源。本管线只产出标准 Markdown，平台特定适配（如 Obsidian wiki-links）由平台适配层处理。
...
- **编码**：UTF-8
- **图形引用**：使用标准 Markdown 语法 `![alt](assets/name.svg)`，平台特定转换（如 Obsidian `![[name.svg]]`）由平台适配层处理

## 风格支持

支持全部 8 种输出风格：论文 / 专栏 / 故事 / 教程 / 观点 / 对话 / 蒸馏Skill / 连载。风格由认知引擎层决定，管线层仅负责格式化落地。
...
- **定位**：最通用管线，是其他管线的文本来源
- **下游复用**：产物可被 HTML / PPT / Notebook / 口播稿等管线复用为文本基础
- **音频管线**：Markdown 文本是音频脚本的来源
- **平台适配**：标准 .md 交付到平台适配层（Obsidian 适配器做 wiki-links 转换、公众号适配器做排版适配等）
