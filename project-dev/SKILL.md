---
name: 项目开发
description: 基于 v2 三阶合一方法论正向设计新项目，从本质定义先行到可运行代码。阶段1（是什么·点出本质）→ 阶段2（为什么·建立因果）→ 阶段3（怎么做·展开方法·视角升级）；类-属性-方法-路由由本质定义锚定；含顿悟触发机制。支持标准模式和迭代模式（Ratchet Loop）。
version: 2.0
scene: C
---
...
- **阶段1（是什么·点出本质）**：抛本质定义 → 本质检验（能解释所有其他属性吗？）→ 类归属（由本质定义锚定，不靠"项目=类"隐喻硬接）→ 关键属性展开（由本质推导，标记✅❌）→ 一句话点破（顿悟触发①）
- **阶段2（为什么·建立因果）**：因果链（起点回溯到阶段1本质定义）→ 反常识根因（顿悟触发②，至少1个反直觉点）→ 技术选型与架构决策（由本质推导）→ 结构性比喻（比喻内部结构与本质同构）
- **阶段3（怎么做·展开方法·视角升级）**：模块划分+接口契约+四层设计+方法设计（本质在动作中的体现）+路由设计（本质的情境化适配）+可运行代码+验证契约 → 视角升级收尾（顿悟触发③，重新定义问题，非总结）

每一层（类/属性/方法/路由）必须能回溯到阶段1的本质定义。
