...
> **方法论引用**：认知方法论（三阶合一、本质定义先行、顿悟触发机制、结构性比喻、思考层/输出层分离）由 [cognitive/general-logic/references/methodology.md](../../../cognitive/general-logic/references/methodology.md) 定义，本文件不重定义。构建完整system prompt时，由执行器（workflow/whiteboard-recorder）读取方法论文件与本文件拼接。
...
按 [本质工坊方法论](../../../cognitive/general-logic/references/methodology.md) 思考（含第九章思考层/输出层分离原则）：思考时走三阶合一，白板分页按视觉节奏自由组织，不强绑三阶分页。
...
*白板生成Prompt设计指南 · 格式管线层 · 方法论引用cognitive · 不重定义认知方法*
